...
object KotlinCompile {
  ...
  lazy val kotlinMemo =
    scalaz.Memo.immutableHashMapMemo[Classpath, KotlinReflection](
  ...
  def compile(options: Seq[String],
    ...
    val javaSources = sourceDirs.filterNot(
      f => sourceDirs.exists(f0 => f0.relativeTo(f).isDefined && f != f0)
    ) map (d => (d, (d ** javaFiles).get)) filter (_._2.nonEmpty)
    if (kotlinSources.isEmpty) {
      ...
      args.freeArgs =
        (kotlinSources ++ javaSources.map(_._1)).map(_.getAbsolutePath).asJava
      args.noStdlib = true
      ...
      args.classpath =
        Option(args.classpath[String]).fold(cp)(_ + File.pathSeparator + cp)
      ...
      args.pluginOptions = Option(args.pluginOptions[Array[String]]).fold(
        kotlinPluginOptions.toArray
      )(_ ++ kotlinPluginOptions.toArray[String])
      output.mkdirs()
...
object KotlinReflection {
  def fromClasspath(cp: Classpath): KotlinReflection = {
    ...
    val messageCollectorClass =
      cl.loadClass("org.jetbrains.kotlin.cli.common.messages.MessageCollector")
...
case class KotlinStub(s: TaskStreams, kref: KotlinReflection) {
  ...
  def messageCollector: AnyRef = {
    ...
    Proxy.newProxyInstance(
      ...
      messageCollectorInvocationHandler
    )
  }
  ...
  def compilerArgs = {
    ...
    new Dynamic {
      def withFirstUpper(string: String): String =
        string.head.toUpper + string.tail
      ...
      def selectDynamic[A](field: String): A = {
        ...
        val getterOpt =
          compilerArgsClass.getMethods.find(_.getName == methodName)
        getterOpt match {
          case Some(getter) => getter.invoke(instance).asInstanceOf[A]
          case None =>
            compilerArgsClass.getField(field).get(instance).asInstanceOf[A]
        }
      }
      ...
      val instance = compilerArgsClass
  ...
  def compile(args: AnyRef): Unit = {
    ...
    val result = compilerExec.invoke(
      compiler,
      messageCollector,
      servicesEmptyField.get(null),
      args: java.lang.Object
    )
    result.toString match {
      case "COMPILATION_ERROR" | "INTERNAL_ERROR" =>
        throw new MessageOnlyException(
          "Compilation failed. See log for more details"
        )
      case _ =>
    }
  }
